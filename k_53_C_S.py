...
# Varijanta: C&S – Correct and Smooth (smooth baze, korekcija rezidualom, ponovo smooth)

"""
Graphs in Space: Graph Embeddings for Machine Learning on Complex Data.
C&S (Correct and Smooth): E_base (spektral) → Smooth (difuzija) → Correct (rezidual y − E@w,
difuzija reziduala, dodaj) → normalizacija u [0, π].
"""
...
CS_ALPHA = 0.5 # 0.5 alpha for CS
CS_ITERS = 15  # 15 iterations for CS
CS_BETA = 0.3  # 0.3 beta for CS
...
def compute_cs_embeddings(M, k=EMBED_DIM):
    ...
    E_base = vecs[:, :k].astype(float)
    E_smooth = E_base.copy()
    for _ in range(CS_ITERS):
        E_smooth = (1.0 - CS_ALPHA) * E_base + CS_ALPHA * (A_norm @ E_smooth)
    deg = A.sum(axis=1).astype(float)
    y = (deg - deg.min()) / (deg.max() - deg.min() + 1e-12)
    w = np.linalg.lstsq(E_smooth, y, rcond=None)[0]
    r = y - (E_smooth @ w)
    r_smooth = r.copy()
    for _ in range(CS_ITERS):
        r_smooth = (1.0 - CS_ALPHA) * r + CS_ALPHA * (A_norm @ r_smooth)
    emb = E_smooth + CS_BETA * r_smooth[:, np.newaxis]
    for d in range(k):
        col = emb[:, d]
...
def main():
    ...
    emb = compute_cs_embeddings(M, k=EMBED_DIM)
    ...
    print()
    print("Predikcija (C&S + kvantna regresija, deterministički, strukturno):")
    print(best_combo)
    print()
    print("Score:", pred_scores[best_combo[0]])
    print()
    """
    Predikcija (C&S + kvantna regresija, deterministički, strukturno):
    (8, 11, 23, 26, 33, 34, 38)

    Score: 0.5037940159868651
    """
...
E_base: prvih k sopstvenih vektora Laplaciana.

Smooth: 
E_smooth = (1−α)·E_base + α·A_norm·E_smooth, iterativno CS_ITERS=15 (CS_ALPHA=0.5) (39×k).

Correct: 
y = normalizovan stepen; w = lstsq(E_smooth, y); r = y − E_smooth@w; r_smooth = difuzija r (ista iteracija); E = E_smooth + CS_BETA·r_smooth (CS_BETA=0.3) (39×k).
